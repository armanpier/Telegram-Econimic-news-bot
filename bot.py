...
def fetch_market_breaking_news(query_keyword):
    ...
    return "\n".join(articles) if articles else "عامل تکنیکال یا لیکوئیدیشن‌های بازار مشتقه (بدون خبر فوری مستقل)"
...
اخبار و بیانیه‌های لحظه‌ای اخیر (شامل مواضع ترامپ، فدرال رزرو، خزانه‌داری آمریکا یا عوامل آنچین/تکنیکال):
{news_context}

یک گزارش تحلیلی ساختاریافته به زبان فارسی در ۴ بخش آماده کن:
1. 🚨 <b>علت و محرک اصلی نوسان</b> (توضیح کوتاه اینکه دقیقا چه خبر، بیانیه یا عاملی باعث این نوسان شد).
2. 📌 <b>جزئیات کلیدی خبر</b> (نکات تکمیلی خبر در ۱ الی ۲ بولت‌پوینت کوتاه).
3. 🔮 <b>پیش‌بینی و سناریوی بعدی (چه خواهد شد؟)</b> (بررسی ۲ سناریو: اگر مومنتوم حفظ شود چه سطحی هدف است، و در صورت اصلاح چه حمایتی مهم است).
4. ⚠️ <b>نکته مدیریت ریسک</b>.

پاسخ را بدون مقدمه و متن اضافه با تگ‌های HTML تمیز بنویس."""
    ...
    for symbol, meta in TRACKED_ASSETS.items():
        ...
        if row:
            ...
            if abs(pct_change) >= threshold:
                ...
                if not recent_alert:
                    logging.info(f"Triggered: {symbol} shifted {pct_change:+.2f}%")
                    ...
                    try:
                        ...
                        logging.info(f"Posted alert for {symbol}")
                        c.execute("INSERT INTO alerts_sent (symbol, pct_change, price, timestamp) VALUES (?, ?, ?, ?)",
                                  (symbol, pct_change, current_price, now_ts))
                        conn.commit()
                    except Exception as tg_err:
                        logging.error(f"Telegram posting error: {tg_err}")

    # Prune price history older than 48 hours
...
async def main():
    if not API_ID or not API_HASH or not TARGET_CHANNEL:
        raise ValueError("Missing essential environment variables (API_ID, API_HASH, TARGET_CHANNEL).")
        
    init_db()
    client = TelegramClient('market_session', API_ID, API_HASH)
    await client.start()
    logging.info("Market Volatility & News Bot active. Polling feeds...")
